...
function bubbleClass(role, contradiction) {
  if (role === "ai") {
    return "self-start bg-[#0f2537] border border-cyan-300/30 text-slate-100";
  }
  if (contradiction) {
    return "self-end bg-rose-500/20 border border-rose-300/50 text-rose-100";
  }
  return "self-end bg-[#1d3448] border border-white/10 text-slate-100";
}
...
function QuestionFlow({ scenario, demoMode, apiBase, onComplete }) {
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[2fr,1fr]">
      <div className="rounded-2xl border border-white/10 bg-steel/55 p-5 shadow-panel">
        ...
        <div className="mb-4 h-2 overflow-hidden rounded-full bg-[#09131e]">
          <motion.div
        ...
        <div className="mb-4 flex h-[420px] flex-col gap-3 overflow-y-auto rounded-xl border border-white/10 bg-[#081321] p-4">
          {messages.map((message, index) => (
            <div key={`${message.role}-${index}`} className={`max-w-[80%] rounded-xl px-4 py-3 text-sm ${bubbleClass(message.role, message.contradiction)}`}>
              <p className="mb-1 text-[10px] uppercase tracking-[0.14em] text-slate-300/80">
        ...
        <div className="rounded-xl border border-white/10 bg-[#0b1b2c] p-3">
          <textarea
            value={answer}
            onChange={handleAnswerChange}
            rows={4}
            placeholder="Enter your response..."
            className="w-full rounded-lg border border-white/10 bg-[#09131e] p-3 text-sm text-slate-100 outline-none ring-neon/50 transition focus:ring"
          />

          <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
            <div className="text-xs text-slate-300">Typing duration: {(typingMs / 1000).toFixed(2)}s</div>
            <button
              type="button"
              disabled={isSubmitting}
              onClick={submitCurrentAnswer}
              className="inline-flex items-center gap-2 rounded-lg bg-neon px-4 py-2 text-sm font-semibold text-[#05212a] disabled:opacity-50"
            >
      ...
      <div className="space-y-4 rounded-2xl border border-white/10 bg-steel/55 p-5 shadow-panel">
        ...
        <div className="grid gap-2 text-sm">
          <div className="rounded-lg border border-white/10 bg-[#09131e] p-3">
            <p className="text-xs uppercase tracking-[0.12em] text-slate-400">Semantic Consistency</p>
            <p className="mt-1 text-lg font-semibold text-white">{(analysis.semantic_consistency * 100).toFixed(1)}%</p>
          </div>

          <div className="rounded-lg border border-white/10 bg-[#09131e] p-3">
            <p className="text-xs uppercase tracking-[0.12em] text-slate-400">Contradiction Count</p>
            ...
            <p className="mt-1 text-xs text-slate-400">{analysis.contradiction_types.join(", ") || "No active contradiction type"}</p>
          </div>
        </div>

        <div className="rounded-lg border border-white/10 bg-[#09131e] p-3 text-xs text-slate-300">
          <p className="mb-2 uppercase tracking-[0.12em] text-slate-400">Context Memory</p>
